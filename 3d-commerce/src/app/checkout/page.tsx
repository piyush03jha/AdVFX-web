...
export default function CheckoutPage() {
  const { items, itemCount, isLoaded } = useCart();
    ...
    <>
      ...
      <main className="min-h-screen overflow-hidden">
        <section className="relative pb-20 pt-4 sm:pb-24 sm:pt-6 lg:pb-28 lg:pt-8">
          <div
            aria-hidden="true"
            className="pointer-events-none absolute left-1/2 top-0 -z-10 h-[460px] w-[460px] -translate-x-1/2 rounded-full bg-primary/[0.05] blur-[140px]"
          />
...
function EmptyCheckout() {
  return (
    <div className="mx-auto max-w-2xl rounded-2xl border border-white/[0.08] bg-white/[0.015] px-6 py-14 text-center sm:px-10">
      ...
      <h2 className="mt-5 font-serif text-3xl tracking-[-0.04em] text-foreground">
        Your cart is empty
      </h2>

      <p className="mx-auto mt-3 max-w-md text-sm leading-6 text-muted">
        Add a model to your cart before continuing to checkout.
      </p>

      <Button asChild className="mt-7 bg-primary text-white hover:bg-primary-hover">
        <Link href="/shop">Explore models</Link>
      </Button>
    </div>
  );
}
